        ...
        $mc="";
           ...
           $offset=$size*$page;
            ...
            if(isset($_GET['motcle'])){
                $mc=$_GET['motcle'];
                $req="SELECT * FROM systeme WHERE Des_syst LIKE '%$mc%' LIMIT $size OFFSET $offset";
            }
            else
            {
                $req="SELECT * FROM systeme LIMIT $size OFFSET $offset";
            }
            $ps=$pdo->prepare($req);
            $ps->execute();
            //Cette requete me permet de connaitre le nbre de page contenant mes enregistrements
            if (isset($_GET['motcle'])){
                $ps2=$pdo->prepare("SELECT COUNT(*) AS NB_ET FROM systeme WHERE Des_syst LIKE '%$mc%'");
            }
            else
            {
                $ps2=$pdo->prepare("SELECT COUNT(*) AS NB_ET FROM systeme");
            }
                $ps2->execute();
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                         ...
                         <h1 style="color:white;font-size: 16px !important;letter-spacing: 0.2em"><i class="fa fa-product-hunt"></i> Liste <span class="table-project-n">des</span> de Système de l'ISPT-KIN <a  onclick="return confirm('Etes-vous sûre...?');" href="vider_crit_syst.php" class="btn btn-danger btn-xs btn-supp"  style="color:white; background-color: #D80027; padding: 3px 8px !important;" title="Supprimer l'enregistrement"><i class="fa fa-trash">Vider</i></a>
                            <a href="new_systeme.php" class="btn btn-primary btn-xs btn-supp" title="Ajouter un enregistrement"><i class="fa fa-plus">Ajouter</i></a>
                        </h1>
                                </div>
                            ...
                            <div class="sparkline13-graph">
                                <div class="datatable-dashv1-list custom-datatable-overright">
                                    <table id="table" data-toggle="table" data-pagination="true" data-search="true" data-show-columns="true" data-show-pagination-switch="true" data-show-refresh="true" data-key-events="true" data-show-toggle="true" data-resizable="true" data-cookie="true"
                                        ...
                                        <thead>
                               <tr>
                                    <th>CODE</th><th>DESIGNATION</th><th>ACTION</th><th>ACTION</th>
                                </tr>
                            </thead>
                            <tbody style="background-color: white;">
                                <?php while ($et=$ps->fetch()){?>
                                <tr>
                                <td><?php  echo($et['cod_syst'])?></td>
                                <td><?php  echo($et['Des_syst'])?></td>
                                <!--liens -->
                                <td><a class="btn btn-danger" onclick="return confirm('Etes-vous sûre...?');" href="SupprimeSysteme.php?id=<?php echo($et['cod_syst'])?>">Supprimer</a></td>
                                <td><a class="btn btn-primary" href="EditSysteme.php?id=<?php  echo($et['cod_syst'])?>">Edit</a> </td>
                                            </tr>   
                                    <?php } ?>  
                            </tbody>
                        </table>
